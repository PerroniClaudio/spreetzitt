...
use App\Mail\ProjectReportUpdatedNotification;
...
class SendProjectPdfReportUpdatedEmail implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (! $company) {
                throw new \Exception('Company not found for project report: ' . $this->report->id);
                return;
            }

            // Carica le relazioni necessarie
            $this->report->load(['company', 'project']);
            ...
            if ($companyAdmins->isEmpty()) {
                throw new \Exception('No company admins found for project report: ' . $this->report->id);
                return;
            }
            ...
            // Listener per confermare l'invio della mail specifica usando l'header X-Project-Report-ID
            $listener = function ($event) use (&$emailSent, $reportId) {
                // Verifica che sia la nostra mail specifica usando l'header custom
                $headers = $event->message->getHeaders();
                if ($headers->has('X-Project-Report-ID')) {
                    $reportIdHeader = $headers->get('X-Project-Report-ID')->getBody();
            ...
            try {
                ...
                Mail::to($recipientEmails)->send(new ProjectReportUpdatedNotification($this->report));
            } finally {
            ...
            if ($emailSent) {
                Log::info('Project report email sent successfully', [
                    'report_id' => $reportId,
                ...
                Log::warning('Project report email was not sent', [
                    'report_id' => $reportId,
                    'recipients' => $recipientEmails,
                ]);
            ...
            Log::error('Failed to send project report email', [
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        Log::error('Project report email job failed after all retries', [
            'report_id' => $this->report->id,
