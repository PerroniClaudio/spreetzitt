...
class SoftwareAssignationsImport implements ToCollection, WithMultipleSheets
{
    ...
    public function collection(Collection $rows)
    {
        ...
        try {
            ...
            foreach ($rows as $row) {
                ...
                if ($this->authUser->is_company_admin && ! $this->canManageAsset($software, $companyToAddId, $companyToRemoveId)) {
                    throw new \Exception('Non puoi modificare le assegnazioni del software indicato.');
                }
                ...
                if (! empty($row[4])) {
                    ...
                    foreach ($usersToRemove as $userToRemove) {
                        ...
                        if ($this->authUser->is_company_admin && ! $this->canAssignUser($user, $software->company_id)) {
                ...
                if (! empty($row[2])) {
                    ...
                    if (count($usersToAdd) > 0) {
                        ...
                        foreach ($usersToAdd as $userToAdd) {
                            ...
                            if (! $this->canAssignUser($user, $software->company_id)) {
    ...
    private function canManageAsset(Software $software, ?int $companyToAddId, ?int $companyToRemoveId): bool
    {
        $companyId = $this->authUser->selectedCompany()?->id;

        return $companyId !== null
            && $software->company_id === $companyId
            && $companyToAddId === null
