...
class HardwareAssignationTemplateExport implements FromArray, WithEvents
{
    /** @var array<int, Company> */
    private array $companies;

    /** @var array<int, Hardware> */
    ...
    public function __construct(User $authUser)
    {
        ...
        $this->hardware = Hardware::query()
            ->whereIn('company_id', $companyIds)
            ->orderBy('id')
            ->get(['id', 'make', 'model', 'company_asset_number', 'support_label'])
            ->all();
    ...
    public function array(): array
    {
        return [[
            ...
            'ID utente/i da associare (solo i numeri separati da virgola)',
            "ID azienda da rimuovere (rimuovendo l'associazione con l'azienda verranno rimosse anche quelle coi rispettivi utenti)",
            'ID utente/i da rimuovere (solo i numeri separati da virgola)',
            "ID responsabile dell'assegnazione",
        ]];
    }
    ...
    public function afterSheet(AfterSheet $event): void
    {
        ...
        $event->sheet->getDelegate()->setTitle('Assegnazioni hardware');
